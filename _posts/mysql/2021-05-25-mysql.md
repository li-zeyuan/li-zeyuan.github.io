...
### 组合索引
数据结构和普通行一样，都是 B+ 树。如组合索引 (A, B, C) 的 B+ 树示意：
```
          (1|2)
         /     \
       A=1     A=2          // 第一层 A 索引
      /   \     /  \
    B=1   B=2  B=1  B=2     // 第二层 B 索引
    / \   / \  / \  / \
  C=1 C=2 C=1 C=2 C=1 C=2   // 第三层 C 索引，
```
- 树的第一层按 A 排序
- 第二层：同一个 A 值下，再按 B 排序
- 第三层： (A, B) 值下，再按 C 排序
...
### 索引下推
在使用二级索引扫描时，把能在索引中判断的条件提前过滤，减少回表次数，提高查询性能。



触发条件：
- 使用二级索引：因为主键索引的叶子节点指向了整行数据
- 查询条件包含索引字段
- 过滤条件能在索引中判断
