...
子context超时，不会影响父context，因为context不可变
```go
func main() {
	ctx := context.Background()
	ctx2, _ := context.WithTimeout(ctx, time.Second*5)
	ctx3, _ := context.WithTimeout(ctx2, time.Second*1)

	<-ctx3.Done()
	fmt.Println("ctx3 Done", time.Now()) // 1s超时

	<-ctx2.Done()
	fmt.Println("ctx2 Done", time.Now()) // 5s超时
}

// ctx3 Done 2026-03-30 16:49:00.844613 +0800 CST m=+1.001404542
// ctx2 Done 2026-03-30 16:49:04.84437 +0800 CST m=+5.001185334
```
