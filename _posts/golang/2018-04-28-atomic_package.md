...
#### 缓存一致性协议（难点）
用来解决多核心 CPU 多级缓存（L1、L2、L3）脏数据问题。最典型是 MESI 协议：
```
M (Modified)：修改过，只有自己有，和内存不一致
E (Exclusive)：只有自己有，和内存一致
S (Shared)：多个核心共享，和内存一致
I (Invalid)：无效
```

实现机制依赖**总线嗅觉**；所有核心监听总线上内存操作，并及时调整缓存状态。
> 参考[缓存一致性（Cache Coherency）入门](https://www.infoq.cn/article/cache-coherency-primer)
